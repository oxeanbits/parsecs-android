...
class ParsecTest {
    ...
    init {
        ...
        assert("2", "sin(1) + cos(0) + tan(0.15722)")
    ...
    private fun assert(expected: String, calc: String) {
        try {
            val result = parsec.eval(calc)
            if(result == expected) {
                Log.i(ParsecTest::class.java.simpleName, "formula: $calc result: [$result]")
            } else {
                Log.e(ParsecTest::class.java.simpleName,
